...
if (!accessToken) {
	console.error(
		"Missing required environment variable: ANNICT_ACCESS_TOKEN",
	);
	process.exit(1);
}

const client = new AnnictClient({ accessToken });
...
if (process.argv.includes("--http")) {
	...
	serve({ fetch: app.fetch, port, hostname: "127.0.0.1" }, (info) => {
		console.error(
			`ruru-annict-mcp server running on http://localhost:${info.port}`,
		);
